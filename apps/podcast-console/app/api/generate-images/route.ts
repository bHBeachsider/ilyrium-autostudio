...
export async function POST(req: Request) {
  ...
  let plan: z.infer<typeof sceneSchema>
  try {
    ...
    console.log("[v0] Scene planning failed:", err instanceof Error ? err.message : err)
    return Response.json({ error: "Failed to plan episode visuals." }, { status: 500 })
  }
  ...
  // 2) Generate each scene image in the chosen style (Imagen 4 Fast, zero-config via AI Gateway).
  try {
    ...
    return Response.json({ visualStyle: plan.visualStyle, images, weights })
  } catch (err) {
